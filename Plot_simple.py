...
from functions import sct_ave_groupby, mov_ave, plt, sct_fit, fit,plot_1 ,plot_2, sctFitAve, sctMVAve
import random as rd
...
def sct_ave_groupby(df, po, col, label):
    ...
    plt.xlabel(label[1])
    plt.ylabel(label[2])
    plt.grid(c='grey', linestyle='dotted', linewidth=0.1, alpha=0.2)
...
#     totmass = np.array(df['LT mass flow rate [kg/s]']) + np.array(df['MT mass flow rate [kg/s]']) + np.array(df['AC mass flow rate [kg/s]'])
    ...
    # v1, label[1] = df['2nd law efficiency Exergy based'], '2nd law efficiency Exergy based'
    ...
    # v2, label[2] = df['stato_level_asp1'],'Liquid level sensor mode'
    # v2, label[2] = df['T3 GC Outlet'], 'T3 GC Outlet'
    ...
    # v2, label[2] = df['AC mass flow rate [kg/s]'], 'AC mass flow rate [kg/s]'
    # v2, label[2] = df['MT mass flow rate [kg/s]'], 'MT mass flow rate [kg/s]'
    # v2, label[2] = df['LT mass flow rate [kg/s]'], 'LT mass flow rate [kg/s]'
    # v2, label[2] = df['Tair external air'], 'Tair external air'
    # v2, label[2] = df[],
    ...
    lpdm = len(PDM)
    for i in range(lpdm):
        leg.append(PDM[i][:pdml] + " average weekly line " )
        weeklist+= (PDM[i][:3])
    for i in range(lpdm):
        # leg.append(PDM[i][:pdml] + " Scatter data ")
        leg.append(PDM[i][:pdml] + " Grouped average based on X axis ")

    print("  Average of  " , PDM[pdm][:3], sum(dff[label[2]]) / (len(dff[label[2]])))
    ...
    # plt.plot(dff)
    # plt.scatterplot(dff)
    plt.scatter(dff[label[1]], dff[label[2]], c=col, marker='.', s=1)
    plt.scatter(dff[label[1]], mov_ave(dff[label[2]].tolist(),300), c='blue', marker='.', s=1)

    plot_2(dff[label[1]].tolist(),dff[label[2]].tolist(), mov_ave(dff[label[2]].tolist()),["Time[Sec]", "Power consumption by MT comressors [W]", "300 sample moveing avarage [W]"], "  ")
