...
from TDN import TDN
from functions import plt, mov_ave, plot_corr, plot_4, print_weekly_ave, print_lengly_ave, is_nan, plot_2, plot_7
...
step_c = step*2
length = 500
weeknum = [28, 29, 31, 32, 33, 34]
# weeknum = [32, 33]
# weeknum = [28]
n = 60    # Moving averaging sample size
"""----------------- %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%------------- """
...
for week in weeknum:
    ...
    label[1], label[2] = 'T1 MT discharge', 'T3 GC Outlet'
    ...
    var2 = dfc[label[2]].tolist()

    # var2 = dfd[label[2]].tolist()
    # vrec = mov_ave(dfd['AK-PC 782A:   Vrec OD'].tolist())
    # p1 =   list(np.array(mov_ave(dfc['P1 discharge'].tolist()))/1e5)
    # p2 =   list(np.array(mov_ave(dfc['P2 receiver'].tolist()))/1e5)
    # sc3 =   list(np.array(mov_ave(dfd['AK-PC 782A:   Sgc ctrl.'].tolist())))
    # p6 =   list(np.array(mov_ave(dfc['P6 MT suction'].tolist()))/1e5)
    # rcmt = mov_ave(dfd['AK-PC 782A:   Running capacity MT'].tolist())
    # rcit = mov_ave(dfd['AK-PC 782A:   Running capacity IT'].tolist())

    # # plot_7(time, vrec, p1, p2, p6, sc3, rcmt, rcit, ["Time [s]",'AK-PC 782A:   Vrec OD','P1 discharge','P2 receiver','P6 MT suction', 'T3 GC Outlet','AK-PC 782A:   Running capacity MT','AK-PC 782A:   Running capacity IT'], week)
    #
    # var1 = dfd['AK-PC 782A:   Sc3'].tolist()
    # var2 = dfd['AK-PC 782A:   Sgc ctrl.'].tolist()
    ...
    #         try:
    #             if -1.5 > var2[i]-var1[i] or var2[i]-var1[i] > 2:
    #                 print(var2[i]-var1[i])
    #                 var1.pop(i)
    #                 var2.pop(i)
    #         except:
    #             i
    #
    # plt.scatter(var1, var2, marker='.', color='b', s=1)

    # plot_2(time, list(np.array(var1)-np.array(var2)), list(np.array(var1)-np.array(var2)), ["Time [s]", label[1], label[2]], week)
plt.show()
