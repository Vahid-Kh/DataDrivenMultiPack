...
def print_weekly_ave(vd, eb, bitzer, direct, dd):
    ...
    print(round(sum(vd) / leng, 6), ' ',
          round(sum(eb) / leng, 6), ' ',
          round(sum(bitzer) / leng, 6), ' ',
          round(sum(direct) / leng, 6), ' ',
          round(sum(dd) / leng, 6))
...
def plot_3(time, var1, var2, var3, label, week):
    ...
    try:
        ...
        time = list(np.array(range(len(var1)))*120)
        host.set_xlim(min(time), max(time))
    span_h = max([max(var1), max(var2), max(var3)]) * 1.1
    span_l = min([min(var1), min(var2), min(var3)]) * 0.9
...
def plot_5(time, var1, var2, var3, var4, var5, label, week):
    ...
    host.legend(lines, [l.get_label() for l in lines])
    # plt.figure(label[1] + ', ' + label[2] + ', ' + label[3] + ', ' + label[4] + '  & ' + label[5] + ', ' + ' as a function of ' + label[0] + ' for week ' + str(week))
