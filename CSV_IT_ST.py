...
# for iiii in [28]:
    ...
    """ X """
    pwrl = mov_ave(pwrlt, int(n/2))[n:]
    mvdl = mov_ave(mlt_cal_vd, int(n/2))[n:]
    mebl = mov_ave(mlt_cal_eb, int(n/2))[n:]
    mbtl = mov_ave(mlt_bitzer, int(n/2))[n:]
    sttl = mov_ave(stl, int(n/2))[n:]
    yl = mov_ave(mlt, int(n/2))[n:]
    time = time[n:]

    if iiii < 32:
        mac = list(mov_ave(dfc['M1 AHU liquid (cooling)'].tolist(), int(n/2)))[n:]
    else:
        mac = list(np.array(mov_ave(dfc['M5 AHU (no ejectors)'].tolist(), int(n/2)))*10)[n:]
    ...
    mebi = mov_ave(mit_cal_eb, n)[n:]
    mbti = mov_ave(mit_bitzer, n)[n:]
    pit = mov_ave(dfc['P5 Aux HP suction'].tolist(), n)[n:]
    pwrit = mov_ave(pwrit, n)[n:]
    pmt = mov_ave(dfc['P6 MT suction'].tolist(), n)[n:]
    pwrmt = mov_ave(pwrmt, n)[n:]
    p3ahu = mov_ave(dfc['P3 AHU evaporation'].tolist(), n)[n:]
    t16 = mov_ave(dfc['T16 Aux suction'].tolist(), n)[n:]
    t14 = mov_ave(dfc['T14 MT suction'].tolist(), n)[n:]
    ...
    dict_df.update({
                    ...
                    'mebi': mebi,
                    'mbti': mbti,
                    'P5 Aux HP suction': pit,
                    'P3 AHU evaporation': p3ahu,
                    'T16 Aux suction': t16,
    ...
    data = data.assign(yl=yl)
    data = data.assign(ym=ym)

    print(data.columns)
    print(n)
    # import matplotlib.pyplot as plt
    # from functions import plot_4
    # plot_4(time,mbti,mebi,mbti,mbti, ['sssssssss','sssssssss','sssssssss','sssssssss','sssssssss',],'sdfs')

    data.to_csv("Data_Calc/Data_" + "_Week_" + str(week))
