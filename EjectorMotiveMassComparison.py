...
for weeknum in [34]:  # For one ejector only
...
# for weeknum in [30,34]:  # For HP ejector only
    ...
    if weeknum == 30:
        ...
        dff = pd.read_csv(r'C:\Users\U375297\Documents\PycharmProjects\Danfoss\Data_30', sep=',',
    ...
    #  ____________________________________
        ...
        #        str(weeknum) + mode[w])
        dfej =pd.read_csv(r"Data_cleaned/W34_CD_LP0_HP1_LE0", sep=',',
                          na_filter=True, skip_blank_lines=True, low_memory=False)
        # capVar = "AK-PC 782A:   Requested capacity"
        capVar = "AK-PC 782A:   Vapor ejector capaci"

        hpejcap = mov_ave(dfej[capVar].tolist(), 46)[46:]


        rmshp = list(np.array(dff['rmshp'])) # Motive to suction mass flow ratio
        rmshp = list(np.array(rmshp)*np.array(hpejcap)/77*2)


        suc_mot_rat = list(mot_m_hp)
        ...
        p_lift = list((np.array(out_p) - np.array(suc_p)) / 1e5)
        sub_cool = list((np.array(df['T3 GC Outlet'].tolist()) - np.array(mot_t)))
        print(np.mean(rmshp),np.mean(mot_m_hp))
        ...
        plt.plot(time, mot_m_hp,'-c')
        plt.plot(dff['time'].tolist(),rmshp,'-r')       # Motive to suction mass flow ratio)
        # plt.plot(time, mac,'-b')
        # plt.plot(time, list(df['mmt']),'-m')
        # plt.plot(time, list(df['mit']),'-g')
        # plt.plot(time, list(np.array(df['M3 MT liquid'])+np.array(df['M4 LT liquid'])), '-y')



        plt.legend(["data driven", "RoM model " + capVar+ " included","Mass flow on AHU","Mass flow on MT comp","Mass flow on IT comp","Load on MT cabin and LT"])
