...
#             leg[i] += " scatter data"
    ...
    # v1, label[1] = df[],
    # v1, label[1] = df[],
    # v1, label[1] = df['Power Meter MT'], 'Power Meter MT[W]'
    #
    ...
    def sct_ave_groupby(df, po, col, label):
        ...
        plt.plot(np.unique(df[label[1]]),
                 np.poly1d(np.polyfit(df[label[1]], df[label[2]], po))(np.unique(df[label[1]])), color=col)
